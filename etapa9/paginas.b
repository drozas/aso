...
############################################################
{
	...
	sys->print("---entrando en tratamiento de get \n");	
...
###################################################################
servirPut (listaPags : ref tCachePags, ruta : string, nuevoContenido :  string) : (int,int)
# - Modifica el contenido de un archivo, tanto en cache como en disco
# - Lo guarda en cache si es la primera vez que lo piden
# - Devuelve -1 en en caso de error de apertura (no existencia)
# - Modif. et.9: devuelve la posicion dnd se encuentra en cache, o la nueva 
#    si no estaba
...
############################################################
{
	...
	sys->print("---entrando en tratamiento de put...\n");	
	...
	if(desc_fichero==nil)
	{
		...
		if(nBytesEscritos==-1)
		{
			...
			if (enCache<0) 
			{
				...
				#MODIFICACIONES ETAPA9: Devolvemos tb la pos dnd esta en cache
				sys->print("no estaba en cache...volvemos a llamar a la funcion q nos dice dnd esta\n");
				pos_aux = estaEnCache(listaPags,ruta);
				sys->print("valor de pos_aux q vamos a devolver...%s \n", string pos_aux);
				...
				#MODIFICACIONES ETAPA9: Devolvemos su posicion en cache
				pos_aux= enCache;
				sys->print("Si estaba en cache!!! valor de pos_aux q vamos a devolver...%s \n", string pos_aux);
...
# - Hilo que se encarga de avisar que se libere una pagina bloqueada, tras 5sg
###################################################################
{
...
###################################################################	
{
	...
	posPut: int; #Recoge el valor devuelto por llamada a tratarPut

	for(;;)
	{
		#Leemos peticiones de threads de cliente por el canuto de peticiones
		sys->print("Thread de cache : esperando a recibir peticiones...\n");


		alt
		{
			#Si leemos del canal de peticiones ....
			peticionCache = <- canal_peticion =>
			{
	
				sys->print("Thread acceso_cache : Recibida peticion de acceso a cache...\n");
				...
				case peticionCache.tipoPeticion {
					"get" => 
						sys-> print("Thread de cache : tratando un get de %s \n", peticionCache.ruta);
						...
						if (error == -2)
						{
							sys->print("Thread de cache : pagina bloqueada...hay que guardar peticion get!\n");
							peticionesPendientes= peticionCache::peticionesPendientes;
						}else{
							sys->print("Thread de cache : la pag no esta bloqueada, enviaremos respuesta a thread tratar_cliente \n");	
					...
					"put" =>
						sys-> print("Thread de cache : tratando peticion put de %s\n", peticionCache.ruta);
						#Llamamos a nuestra antigua funcion servirPut, con la info de la peticion
						(error, posPut) = servirPut(listaPags,peticionCache.ruta,peticionCache.nuevoContenido);
						#Hacemos control de errores solo de peticiones bloqueadas
						if (error==-2)
						{
							sys->print("Thread de cache : pagina bloqueada...hay que guardar peticion put!\n");
							peticionesPendientes = peticionCache::peticionesPendientes;
							...
							respuestaCache.error = error;
							#Enviamos la respuesta por el canal de respuesta(de cada thread!)
							sys -> print("Thread de cache : enviando respuesta a put \n");
							peticionCache.canalRespuesta <- = respuestaCache;
		
							sys ->print("lanzando thread lease //b.inicial// para pos %s\n", string posPut);
							#Y lanzamos un thread de lease con esa pos
							spawn lease(posPut, canal_lease);
						}


					"end" =>
						#Solo desbloquea en estructura, la liberacion de recursos se produce abajo
						sys-> print("Thread de cache : tratando peticion end de %s\n", peticionCache.ruta);
						#Miramos si hay alguna pag en cache con ese nombre para desbloquear
						pos = estaEnCache(listaPags,peticionCache.ruta);
						if(pos<0)
						{
							sys->print("Thread de cache : Nos han hecho un end de una pagina que no tenemos en cache\n");
							respuestaCache.tipoRespuesta="end";
							respuestaCache.contenido = nil;
							respuestaCache.error= -1;
							#Enviamos la respuesta por el canal de respuesta(de cada thread!)
							sys -> print("Thread de cache : ENVIANDO RESPUESTA A UN END ERRONEO DESDE THREAD DE CACHE \n");
	
						}else{
							sys->print("Thread de cache : Se ha encontrado una pagina a desbloquear!!!\n");
							...
							respuestaCache.error= 1;
							#Enviamos la respuesta por el canal de respuesta(de cada thread!)
							sys -> print("Thread de cache : enviando respuesta a un end \n");
						}
			...
			{
				sys->print("Thread de acceso a cache : recibida peticion de desbloqueo LEASE \n");
				#Si estaba bloqueada la desbloqueo, si no...es que ya habra llegado el end
				if(listaPags.pag[posLease].bloqueada==-1)
				{
					sys->print("pagina en pos : %s estaba bloqueada ...haremos efectivo el lease!!!!\n", string posLease);
					listaPags.pag[posLease].bloqueada=1;
				}else{
					sys->print("pagina en pos : %s NO estaba bloqueada, despreciaremos el lease!!!", string posLease);
				}

			}
		}#estructura_alt

		sys->print("Estado de cache de pags, antes de liberacion...\n");
		sys->print("/////////////////////////////////////////////////////////////\n");
		...
		while(listaInversa!=nil)
		{
			...
			case petiAux.tipoPeticion {
				...
				"put" =>
					...
					#Llamamos a nuestra antigua funcion servirPut, con la info de la peticion
					(error,posPut) = servirPut(listaPags,petiAux.ruta,petiAux.nuevoContenido);
					#Hacemos control de errores solo de peticiones bloqueadas
					if (error==-2)
					{
						...
						respuestaCache.error = error;
						#Enviamos la respuesta por el canal de respuesta(de cada thread!)
						sys -> print("ENVIANDO RESPUESTA A UN PUT DESDE THREAD DE CACHE \n");
						petiAux.canalRespuesta <- = respuestaCache;
						sys ->print("lanzando thread lease //b.secundario// para pos %s\n", string posPut);
						#Y lanzamos un thread de lease con esa pos
						spawn lease(posPut, canal_lease);
					}
		...
		sys->print("Estado de cache de pags, despues de liberacion...\n");
		sys->print("/////////////////////////////////////////////////////////////\n");
		pintartCachePags(listaPags);
		sys->print("/////////////////////////////////////////////////////////////\n");
...
###################################################################
{
	...
	for(;;)
	{
		...
		sys->print("****Thread de conexiones: esperando a recibir peticiones\n");
		aux = <- canal_entrada;
		sys->print("****Thread de conexiones: recibido el valor -> %s\n", string aux);
		...
		if(nConexiones==0) 
		{
			...
			if (fd==nil)
			{
				sys->print("check_conexiones: Descriptor de grupo vacio!\n");
			}else{
...
###################################################################
{
	sys->print("Kill´em all! : Me duermo 15 sg...\n");
	sleep(15000);
	sys->print("Kill´em all! : os voy a matar a todos!\n");

	#Abrimos el descriptor, y le escribimos señal killgrp
	fd:=open("/prog/" + string gid + "/ctl", OWRITE);
	if (fd==nil)
	{
		sys->print("Kill´em all!: Descriptor de grupo vacio!\n");
	}else{
		write(fd, array of byte "killgrp", 7);	
	}
	exit;
