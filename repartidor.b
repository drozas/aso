...
# - Se comunica con politica y checkea_conexiones mediante canales, para evitar cond. de carrera
###############################################################################
{
	...
	#Variables para coordinacion de actualizaciones
	peticionLiberacionUnpack : ref Reqmsg;
	recursoLiberado : string;
	peticionLiberacion : array of byte;

	#Variables para comunicacion con thread politica
	peticionPolitica :tPeticionPolitica;
	...
	for(;;)
	{
		...
		if (es != nil)
		{
			sys->print("repartidor->tratar_cliente: Fin de conexion con cliente.\n");
			sys->print("------------------------------------------\n");
			canal_conexiones<-=-1;
			exit;
		}else{
			...
			if (es!=nil)
			{
				sys->print("repartidor->tratar_cliente: Error al desempaquetar.\n");
				exit;
			}else{
				...
				pick r := peticionUnpack {
					...
					Get =>
						...
						#Y lo haremos hasta que uno funcione (tolerancia a fallos)
						do
						{
							...
							if ((e1<0)||(e2<0))
							{
								...
								#Preguntamos al thread por el siguiente server por el mismo canal
								canal_politica <- = peticionPolitica;
								respuestaPolitica=<-peticionPolitica.canal_respuesta;
							}
...
# - Le pasamos el array de bytes, la peticion ha sido leida en el thread
...
# - Accede a listaServers solo en modo lectura
###################################################################
{
	i: int;
...
# - Accede a lista servers solo para leer
...
# - Hace un receive End de todos los servers
# - Accede a la listaServers en modo lectura
...
# - Prepara la cadena addr para el formato de la conexion
#############################################
{
...
init(nil: ref Draw->Context, argv: list of string)
{
	...
	#Y nuestra direccion de repartidor
