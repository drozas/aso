...
#El identificador direcion se guarda sin "/", Ej.:
...
# - Devuelve -1 si no encontro a ninguno, -2 si no hay ningun server
##############################################################
{
...
# - Le pasamos el descriptor de conexion
##############################################################
{
	(buf, e) := readmsg(in, 10*TAM_BLOQUE);

	#Controlamos q se lea correctamente
	if (e != nil)
	{
...
# - Prepara la cadena para  el formato de la conexion
###############################################
{
...
# - Realiza la conexion con un paginas.
	...
	for(;;)
	{
		...
		if (es != nil)
		{
			...
			if (es != nil)
			{
				...
				if (pos>0) 
				{
					#Ahora, se lo enviaremos al correspondiente 
					if (pos==0)
					{	
...
###################################################################
{
	...
	for(;;)
	{
		...
		#Si es 0, lanzo el thread asesino
		if(nConexiones==0) 
		{
			spawn killemall(gid);
		}else{
